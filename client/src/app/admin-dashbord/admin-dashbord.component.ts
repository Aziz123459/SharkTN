...
export class AdminDashbordComponent {
  ...
  filterUsersByType(): void {
    ...
    this.startups = this.users.filter(user => user.acctype === 'startup');
    this.investors = this.users.filter(user => user.acctype === 'investor');
  }
    ...
    if(acctype=='investor'){
      this.apiService.getInvestorByUserId(userId).subscribe({
        next: (data:Investor) => {
          ...
          let investerid=this.investor._id
        ...
        this.apiService.getStartupByUserId(userId).subscribe({
          next: (data:Startup) => {
            ...
            let startupid=this.startup._id
